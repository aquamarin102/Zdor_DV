...
public class GenerationLoc : MonoBehaviour
{
    ...
    {
        var terrain =
            Terrain.CreateTerrainGameObject(GenerationTerrain()).GetComponent<Terrain>();
    ...
    private SplatPrototype[] GenerationSplatPrototype()
    {
        ...
        SplatPrototype[] splatPrototypes = new
            SplatPrototype[_terrainTextures.Length];
    ...
    {
        var map = new float[t.terrainData.alphamapWidth,
            t.terrainData.alphamapHeight, 2];
        for (var y = 0; y < t.terrainData.alphamapHeight; y++)
        {
            for (var x = 0; x < t.terrainData.alphamapWidth; x++)
            {
                ...
                var angle = t.terrainData.GetSteepness((float)normX,
                    (float)normY);
                var frac = angle / 90.0;
